...
class TestingStreamsSpec
  ...
  "A simple stream" should {
    "satisfy basic assertions" in {
      ...
      val simpleSink = Sink.fold(0)((e: Int, a: Int) => e + a)
    ...
    "integrate with test-actor-based sink" in {
      ...
      // sink from test probe
      val probeSink =
        Sink.actorRef(probe.ref, "completedMessage", PartialFunction.empty)
    ...
    "integrate with Streams TestKit Source" in {
      ...
      val (testPublisher, resultFuture) =
